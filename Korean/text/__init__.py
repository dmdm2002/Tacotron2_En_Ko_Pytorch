...
from Korean.text import cleaners
from Korean.ko_hparams import ko_hparams
from Korean.text.symbols import symbols, en_symbols, PAD, EOS
from Korean.text.korean import jamo_to_korean
...
def text_to_sequence(text, as_token=False):
    cleaner_names = [x.strip() for x in ko_hparams.text_cleaners]
...
def sequence_to_text(sequence, skip_eos_and_pad=False, combine_jamo=False):
    ...
    cleaner_names = [x.strip() for x in ko_hparams.cleaners.split(',')]
    if 'english_cleaners' in cleaner_names and isEn == False:
        convert_to_en_symbols()

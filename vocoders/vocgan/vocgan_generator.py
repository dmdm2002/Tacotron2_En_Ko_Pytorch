...
class Generator(nn.Module):
    ...
    def remove_weight_norm(self):
        """Remove weight normalization model from all of the layers."""

        def _remove_weight_norm(m):
            try:
                torch.nn.utils.remove_weight_norm(m)
            except ValueError:  # this model didn't have weight norm
                return

        self.apply(_remove_weight_norm)

    def apply_weight_norm(self):
        """Apply weight normalization model from all of the layers."""

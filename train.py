...
from Korean.ko_hparams import ko_hparams as ko_hps
from English.en_hparams import en_hparams as en_hps
from utils.logger import Tacotron2Logger
from utils.dataset import KoreanDataset, EnglishDataset, Collate
from model.builder import get_tacotron2, Tacotron2Loss
...
def prepare_dataloaders(fdir, n_gpu, language):
    if language == 'korean' or language =='ko':
        trainset = KoreanDataset(fdir)
    else:
        trainset = EnglishDataset(fdir)
    collate_fn = Collate(hps.n_frames_per_step)
...
def train(args):
    ...
    if args.language == 'korean' or args.language =='ko':
        eg_text = ko_hps.eg_text
    else:
        eg_text = en_hps.eg_text

    # build model
    model = get_tacotron2(args.language)
    mode(model, True)
    if n_gpu > 1:
    ...
    # make dataset
    train_loader = prepare_dataloaders(args.data_dir, n_gpu, args.language)
        ...
        for batch in train_loader:
            ...
            if rank == 0:
                ...
                if args.log_dir != '' and (iteration % hps.iters_per_sample == 0):
                    model.eval()
                    output = infer(eg_text, model.module if n_gpu > 1 else model)
                    model.train()
...
if __name__ == '__main__':
    ...
    # path
    parser.add_argument('-d', '--language', type=str, default='korea',
                        help='data langauge')
    ...
    parser.add_argument('--ckpt_pth', type=str, default='',
                        help='path to load checkpoints, 만약 없으면 비워둬 주세요')
